...
def get_clubs(session: Session, payload: GetClubsPayload) -> List[ClubSchema]:
    return []
    # query = session.query(Club).join(Table, Table.club_id == Club.id).filter(Club.is_active == True)
    ...
    # schema_results: List[ClubSchema] = [ClubSchema.model_validate(club) for club in results]
